...
import { checkAvailability, createReservation, fetchBlockedRanges } from '../services/reservations';
            ...
            <Stack spacing={3}>
              <Paper variant="outlined" sx={{ p: 2.5, borderRadius: 3 }}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', minHeight: 56 }}>
                      {dateDebutText && dateFinText ? (
                        <>
                          <Chip label={form.dateDebut?.format('D MMM')} color="primary" variant="outlined" size="small" />
                          <Typography variant="caption" color="text.secondary">→</Typography>
                          <Chip label={form.dateFin?.format('D MMM')} color="primary" variant="outlined" size="small" />
                          {nights > 0 && (
                            <Chip label={`${nights} nuit${nights > 1 ? 's' : ''}`} size="small" color="primary" sx={{ fontWeight: 700 }} />
                          )}
                        </>
                      ) : (
            ...
            <Stack spacing={2} sx={{ position: { md: 'sticky' }, top: { md: 24 } }}>
              ...
              <Stack spacing={1.5}>
                {activeStep === 0 ? (
                  ...
                  <>
                    <Button fullWidth variant="outlined" onClick={() => setActiveStep(0)} disabled={loading} sx={{ borderRadius: 2 }}>
                      ← Modifier le séjour
                    </Button>
                    <Button
                      fullWidth
