...
describe('ModifierStatutTenantUseCase', () => {
  ...
  it('TEST-TENANT-05: lève BadRequestException "Statut invalide"', async () => {
    await expect(useCase.execute('tenant-uuid', 'INCONNU')).rejects.toThrow(
  ...
  it('TEST-TENANT-06: lève NotFoundException "Tenant introuvable"', async () => {
    ...
    await expect(useCase.execute('uuid-inexistant', 'ACTIF')).rejects.toThrow(
      new NotFoundException('Tenant introuvable'),
    );
  });
});
