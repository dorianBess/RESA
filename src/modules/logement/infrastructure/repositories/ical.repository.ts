...
export class IcalRepository implements IIcalRepository {
  ...
  async saveUrls(logementId: string, _tenantId: string, data: { urlIcalAirbnb?: string; urlIcalBooking?: string }): Promise<void> {
  ...
  async getUrls(logementId: string): Promise<{ urlIcalAirbnb?: string; urlIcalBooking?: string } | null> {
    const l = await this.repo.findOne({ where: { id: logementId } });
    if (!l) return null;
    return { urlIcalAirbnb: l.urlIcalAirbnb ?? undefined, urlIcalBooking: l.urlIcalBooking ?? undefined };
  }
}
