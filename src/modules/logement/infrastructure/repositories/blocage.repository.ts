...
export class BlocageRepository implements IBlocageRepository {
  constructor(
    ...
    private readonly repo: Repository<BlocageDatesEntity>,
  ...
  async findByLogement(
    ...
    const qb = this.repo
      ...
      .where('b.logementId = :logementId AND b.tenantId = :tenantId', {
        logementId,
        tenantId,
      });
    if (opts?.source)
    ...
    if (opts?.dateDebut)
      qb.andWhere('b.dateFin >= :dateDebut', { dateDebut: opts.dateDebut });
    if (opts?.dateFin)
      qb.andWhere('b.dateDebut <= :dateFin', { dateFin: opts.dateFin });
  ...
  async existsConflictWithReservation(
    logementId: string,
    dateDebut: Date,
    dateFin: Date,
  ): Promise<boolean> {
  ...
  async create(
    data: Omit<BlocageDomain, 'id' | 'createdAt'>,
  ): Promise<BlocageDomain> {
    const entity = this.repo.create(
      data as any,
    ) as unknown as BlocageDatesEntity;
  ...
  private toDomain(b: BlocageDatesEntity): BlocageDomain {
    return {
      id: b.id,
      logementId: b.logementId,
      tenantId: b.tenantId,
      dateDebut: b.dateDebut,
      dateFin: b.dateFin,
      source: b.source,
      motif: b.motif ?? undefined,
      statut: b.statut,
