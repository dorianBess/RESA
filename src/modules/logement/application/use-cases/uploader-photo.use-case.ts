...
export class UploaderPhotoUseCase {
  ...
  async execute(command: UploaderPhotoCommand): Promise<PhotoDomain> {
    ...
    if (command.size > TAILLE_MAX_OCTETS) {
      throw new BadRequestException('Fichier trop volumineux. Taille maximale : 5Mo');
    }
    ...
    const url = await this.storageService.upload(command.buffer, command.originalname, command.mimetype);
