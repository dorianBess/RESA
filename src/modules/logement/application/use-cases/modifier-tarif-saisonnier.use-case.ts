...
export class ModifierTarifSaisonnierUseCase {
  ...
  async execute(id: string, tenantId: string, data: Partial<TarifSaisonnierDomain>): Promise<TarifSaisonnierDomain> {
    const updated = await this.tarifRepository.updateSaisonnier(id, tenantId, data);
