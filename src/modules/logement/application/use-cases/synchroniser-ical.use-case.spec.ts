...
import { IIcalRepository, IIcalFetcher, INotificationService } from '../../domain/ports/ical.repository.port';
...
describe('SynchroniserIcalUseCase', () => {
  ...
  const event1 = { uid: 'e1', dateDebut: new Date('2025-07-10'), dateFin: new Date('2025-07-15') };
  const event2 = { uid: 'e2', dateDebut: new Date('2025-08-01'), dateFin: new Date('2025-08-05') };
  ...
  beforeEach(() => {
    ...
    useCase = new SynchroniserIcalUseCase(mockIcalRepo, mockFetcher, mockBlocage, mockDispo, mockNotif);
  });
  ...
  it('TEST-ICAL-03: crée 2 blocages source AIRBNB depuis le fichier iCal', async () => {
    mockIcalRepo.getUrls.mockResolvedValue({ urlIcalAirbnb: 'https://airbnb.fr/ical/xxx.ics' });
    ...
    mockBlocage.create.mockResolvedValue({
      ...
      dateDebut: event1.dateDebut, dateFin: event1.dateFin, source: SourceBlocage.AIRBNB,
    });
  ...
  it('TEST-ICAL-04: crée blocage CONFLIT et envoie email si chevauchement avec réservation RESA', async () => {
    const conflictEvent = { uid: 'conflict', dateDebut: new Date('2025-07-17'), dateFin: new Date('2025-07-22') };
    mockIcalRepo.getUrls.mockResolvedValue({ urlIcalAirbnb: 'https://airbnb.fr/ical/xxx.ics' });
    ...
    mockBlocage.create.mockResolvedValue({
      id: 'b-uuid', logementId: 'log-uuid', tenantId: 'tenant-A',
      dateDebut: conflictEvent.dateDebut, dateFin: conflictEvent.dateFin,
      source: SourceBlocage.AIRBNB, statut: 'CONFLIT',
    });
    ...
    expect(mockNotif.sendConflitIcalAlert).toHaveBeenCalledWith(
      'test@gite.fr', 'Gîte', conflictEvent.dateDebut, conflictEvent.dateFin,
    );
  ...
  it('TEST-ICAL-05: ne crée pas de doublon si blocage existe déjà', async () => {
    ...
    mockBlocage.findByDateRange.mockResolvedValue([{
      id: 'existing', logementId: 'log-uuid', tenantId: 'tenant-A',
      dateDebut: event1.dateDebut, dateFin: event1.dateFin, source: SourceBlocage.AIRBNB,
    }]);
