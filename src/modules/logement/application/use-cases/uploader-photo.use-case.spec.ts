...
describe('UploaderPhotoUseCase', () => {
  ...
  const logement = {
    id: 'logement-uuid', tenantId: 'tenant-A', nom: 'Gîte', capacite: 4,
  ...
  beforeEach(() => {
    ...
    useCase = new UploaderPhotoUseCase(mockLogementRepo, mockPhotoRepo, mockStorage);
  });
  ...
  it('TEST-PHOTO-01: retourne photo avec URL S3 et ordre 0', async () => {
    ...
    mockStorage.upload.mockResolvedValue('https://s3.eu-west-3.amazonaws.com/resa-dev/photo.jpg');
    mockPhotoRepo.save.mockResolvedValue({
      id: 'photo-uuid', logementId: 'logement-uuid', tenantId: 'tenant-A',
      url: 'https://s3.eu-west-3.amazonaws.com/resa-dev/photo.jpg', ordre: 0,
    });
  ...
  it('TEST-PHOTO-02: lève BadRequestException "Format non supporté"', async () => {
    await expect(
      useCase.execute({ ...baseCmd, mimetype: 'application/pdf', originalname: 'doc.pdf' }),
    ).rejects.toThrow(new BadRequestException('Format non supporté. Formats acceptés : jpg, png, webp'));
  });
  ...
  it('TEST-PHOTO-03: lève BadRequestException "Fichier trop volumineux"', async () => {
    ...
    ).rejects.toThrow(new BadRequestException('Fichier trop volumineux. Taille maximale : 5Mo'));
  });
});
