...
export class VerifierDisponibiliteCompletUseCase {
  constructor(
    ...
    private readonly configAcompteRepository: IConfigAcompteRepository,
  ) {}
  ...
  async execute(
    ...
    if (command.nbPersonnes > logement.capacite) {
      throw new ConflictException(
        `Capacité insuffisante pour ${command.nbPersonnes} personnes`,
      );
    }
    ...
    const prixParNuit =
      tarifSaisonnier?.prixParNuit ?? tarifBase?.prixParNuit ?? 0;
    const montantTotal = nbNuits * prixParNuit;

    // Config acompte
    const configAcompte = await this.configAcompteRepository.findByLogement(
      command.logementId,
    );
    const acompteActif = configAcompte?.actif ?? false;
    const montantAcompte =
      acompteActif && configAcompte
