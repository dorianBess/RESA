...
describe('ConfirmerPaiementUseCase', () => {
  ...
  const resa = {
    id: 'resa-uuid', tenantId: 'tenant-A', statut: 'EN_ATTENTE',
    montantTotal: 840, stripePaymentIntentId: 'pi_valid',
  };
  ...
  beforeEach(() => {
    ...
    mockResaRepo = {
      findByPaymentIntentId: jest.fn(), updateStatut: jest.fn(),
    };
    mockStripe = {
      createPaymentIntent: jest.fn(), retrievePaymentIntent: jest.fn(), refund: jest.fn(),
    };
    useCase = new ConfirmerPaiementUseCase(mockPaiementRepo, mockResaRepo, mockStripe);
  });
  ...
  it('TEST-PAIE-01: paiement CAPTURE + réservation CONFIRMEE', async () => {
    ...
    mockStripe.retrievePaymentIntent.mockResolvedValue({ id: 'pi_valid', status: 'succeeded', amount: 84000 });
    ...
    expect(mockResaRepo.updateStatut).toHaveBeenCalledWith('resa-uuid', 'CONFIRMEE');
  });
  ...
  it('TEST-PAIE-04: paiement type ACOMPTE CAPTURE quand acompte activé', async () => {
    ...
    const paiementAcompte = { ...paiementCapture, type: TypePaiement.ACOMPTE, montant: 252 };
    mockPaiementRepo.findByPaymentIntentId.mockResolvedValue(null);
    mockResaRepo.findByPaymentIntentId.mockResolvedValue(resaAvecAcompte);
    mockStripe.retrievePaymentIntent.mockResolvedValue({ id: 'pi_valid', status: 'succeeded', amount: 25200 });
